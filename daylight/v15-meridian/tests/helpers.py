...
def witness(label: str) -> dict[str, str]:
    return {
        "witness_type": label,
        "witness_digest": canonical_sha256({"witness": label}, "DAYLIGHT-v15-MERIDIAN-TEST-WITNESS:"),
    }


def transcript(label: str) -> str:
    return canonical_sha256({"transcript": label}, "DAYLIGHT-v15-MERIDIAN-TEST-TRANSCRIPT:")


def artifact(label: str) -> str:
    return canonical_sha256({"artifact": label}, "DAYLIGHT-v15-MERIDIAN-TEST-ARTIFACT:")
