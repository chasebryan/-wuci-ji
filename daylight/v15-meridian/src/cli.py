...
def cmd_frontier(args: argparse.Namespace) -> int:
    ...
    weights = scoring.load_weights(DEFAULT_WEIGHTS)
    labels = obligation_model.labels(registry)
    internal = scoring.compute_score(obligation_model.internal_ceiling_q_vector(registry), weights, labels)
    perfect = scoring.compute_score(obligation_model.perfect_q_vector(registry), weights, labels)
    external_rows = [
        {"obligation_id": ob["id"], "q_id": q_id, "weight": int(ob["weight"]), "external_role": ob.get("external_role", "")}
        for q_id, ob in obligation_model.iter_obligations(registry)
    ...
    report = {
        ...
        "external_residue_M": perfect["final_score_M"] - internal["final_score_M"],
        "external_obligations": sorted(external_rows, key=lambda row: row["obligation_id"]),
        "note": "perfect_score_M is reachable only by closing every external obligation with a non-harness external attestation; the harness cannot self-certify these.",
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Daylight v15 Meridian execution package")
