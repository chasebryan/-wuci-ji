...
class ScorecardDigestTests(unittest.TestCase):
    def _card(self) -> tuple[dict, dict, dict]:
        fields = registry.load_fields_registry()
        state = scorecard.load_state(BASELINE_STATE)
        return scorecard.build_scorecard(state, fields), state, fields

    def test_generated_scorecard_verifies(self) -> None:
        card, state, fields = self._card()
        scorecard.verify_scorecard_object(card, state, fields)
