#!/usr/bin/env python3
"""Run WuciOS Euclid Phase 3C-D Yocto preparation policy checks."""
...
from yocto_prep_common import (
    ...
    validate_yocto_metadata_policy,
    write_json,
    write_markdown,
)
...
PHASE_NAME = "WuciOS v2.4 Euclid Trial Phase 3C-D — Yocto Layer/Recipe Preparation"
AUTH_ENV = "WUCIOS_PHASE3CD_ALLOW_L2_SCAFFOLD"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--buildroom-output-dir", default="build/wucios/buildrooms/yocto-layer/phase-3c-d")
    parser.add_argument("--buildrooms-dir", default="wucios/buildrooms/yocto-layer")
    parser.add_argument("--guardrails", action="store_true", help="Run Phase 3C-D negative guardrail checks")
    return parser.parse_args()


def validate_plan(plan: dict[str, Any]) -> None:
    if plan.get("phase_id") != PHASE_ID:
        raise SystemExit("Phase 3C-D plan has wrong phase_id")
    if plan.get("phase_name") != PHASE_NAME:
        raise SystemExit("Phase 3C-D plan has wrong phase_name")
    if plan.get("default_execution_mode") != "L1_YOCTO_POLICY_AND_METADATA_INPUTS":
        raise SystemExit("Phase 3C-D plan must default to L1 Yocto policy checks")
    if plan.get("l2_scaffold_authorized_by_default") is not False:
        raise SystemExit("Phase 3C-D plan must disable L2 scaffold by default")
    if plan.get("l3_substrate_artifact_attempts_allowed") is not False:
        raise SystemExit("Phase 3C-D plan must forbid L3 artifact attempts")
    if plan.get("runtime_inspection_allowed") is not False:
        raise SystemExit("Phase 3C-D plan must forbid runtime inspection")
    if plan.get("substrate_selection") != "NO_SUBSTRATE_SELECTED":
        raise SystemExit("Phase 3C-D plan must preserve NO_SUBSTRATE_SELECTED")
    for key in [
        ...
        if plan.get(key) is not False:
            raise SystemExit(f"Phase 3C-D plan must set {key} false")
    if list(plan.get("in_scope_candidates", [])) != CANDIDATES:
        raise SystemExit("Phase 3C-D plan has wrong in-scope candidate set")


def validate_policies(buildrooms_dir: Path) -> dict[str, Any]:
    plan = load_json(buildrooms_dir / "euclid-yocto-phase-3c-d.json")
    validate_plan(plan)
    yocto_policy = load_json(buildrooms_dir / "yocto-layer-policy.json")
    metadata_policy = load_json(buildrooms_dir / "yocto-metadata-input-policy.json")
    ...
    policy_failures = validate_yocto_metadata_policy(metadata_policy)
    if policy_failures:
    ...
    for candidate in CANDIDATES:
        ...
        for key in [
            ...
            if policy.get(key) is not False:
                raise SystemExit(f"{candidate} preparation policy must set {key} false")
        candidate_policies[candidate] = policy

    return {
        "plan": plan,
        "yocto_policy": yocto_policy,
...
def candidate_report(candidate: str, policy: dict[str, Any], scaffold_paths: list[str]) -> dict[str, Any]:
    input_detection = detect_yocto_inputs(candidate, policy)
    ...
    return {
        ...
        "yocto_input_type": str(policy.get("yocto_input_type", "NOT_MEASURED")),
        ...
        "phase_3c_d_outputs": list(policy.get("phase_3c_d_allowed_outputs", [])),
        "blocked_until": list(policy.get("blocked_until", [])),
        "scaffold_paths": scaffold_paths,
        "notes": [
            "Phase 3C-D report only; no BitBake execution, Yocto build, rootfs generation, or image generation occurred.",
...
def run_guardrails(output_root: Path) -> dict[str, Any]:
    guardrail_dir = output_root / "guardrail-tests/phase-3c-d"
    ensure_directory(guardrail_dir)
    env = os.environ.copy()
    env.pop(AUTH_ENV, None)
    env.pop("WUCIOS_EUCLID_ALLOW_ATTEMPT", None)
    env.pop("WUCIOS_PHASE3CC_ALLOW_L2_SCAFFOLD", None)

    checks: list[dict[str, Any]] = []
    commands = [
        ("l2-yocto-scaffold-without-authorization", ["make", "wucios-euclid-yocto-phase-3c-d-scaffold"]),
        ("phase-2-attempt-without-authorization", ["make", "wucios-euclid-trial-phase-2-attempt"]),
        ("phase-3c-c-scaffold-without-authorization", ["make", "wucios-euclid-store-root-phase-3c-c-scaffold"]),
    ]
    ...
    checks.append({
        ...
        "expected": "no forbidden execution patterns in default Phase 3C-D paths",
        "status": scan["status"],
        "violations": scan["violations"],
    })

    report = {
        "schema": "wucios.euclid.phase3c_d.guardrails.v1",
        "phase_id": PHASE_ID,
...
def load_existing_guardrails(output_root: Path) -> list[dict[str, Any]]:
    path = output_root / "guardrail-tests/phase-3c-d/guardrail-report.json"
    if not path.is_file():
        return []
    try:
...
def build_report(args: argparse.Namespace, policies: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if env_authorized and not args.l2_scaffold:
        notes.append("L2_YOCTO_SCAFFOLD_ENV_PRESENT_BUT_FLAG_ABSENT")
    if args.l2_scaffold and not env_authorized:
        notes.append("L2_YOCTO_SCAFFOLD_NOT_AUTHORIZED")
        blocked = True
    ...
    if scaffold_authorized:
        for candidate in requested_candidates:
            scaffold_by_candidate[candidate] = generate_scaffold(
                ...
                policies["metadata_policy"],
                policies["evidence"],
                ROOT / "build/wucios/buildrooms/yocto-layer",
            )
    ...
    if blocked:
        status = "PHASE_3C_D_BLOCKED"
        execution_mode = "L2_YOCTO_SCAFFOLD_NOT_AUTHORIZED" if args.l2_scaffold and not env_authorized else "PHASE_3C_D_GUARDRAIL_BLOCKED"
    elif scaffold_authorized:
        status = "PHASE_3C_D_SCAFFOLD_COMPLETE"
        execution_mode = "L2_YOCTO_NON_ARTIFACT_SCAFFOLD_AUTHORIZED"
    else:
        status = "PHASE_3C_D_RULES_COMPLETE"
        execution_mode = "L1_YOCTO_POLICY_AND_METADATA_INPUTS"
    ...
    report = {
        "schema": "wucios.euclid.phase3c_d.v1",
        "phase_id": PHASE_ID,
        ...
        "yocto_build_attempted": False,
        "bitbake_execution_attempted": False,
        "build_environment_initialization_attempted": False,
        ...
        "layer_download_attempted": False,
    ...
    write_json(output_dir / "euclid-trial-phase-3c-d.json", report)
    write_markdown(output_dir / "euclid-trial-phase-3c-d.md", markdown)
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "In-Scope Candidates: Yocto",
        "Out-of-Scope Preserved: Buildroot/Alpine/Debian minimal/Void -> Phase 3C-B; NixOS/Guix -> Phase 3C-C; OpenBSD reference -> Phase 3C-E",
        "Substrate Artifact Attempt: false",
        "Rootfs Generation: false",
        "Image Generation: false",
        "Yocto Build: false",
        "BitBake Execution: false",
        ...
        "## Purpose",
        "",
        "Phase 3C-D defines Yocto layer, recipe, machine, distro, metadata, and image-construction preparation rules without building, downloading, cloning, executing, imaging, or scoring anything.",
        "",
        "## Build Room Rule",
        "",
        ...
        "## Scope Boundary",
        "",
        "Buildroot, Alpine, Debian minimal, and Void remain Phase 3C-B direct-rootfs candidates. NixOS and Guix remain Phase 3C-C store-root candidates. OpenBSD reference remains deferred to Phase 3C-E. None are executed or downgraded by this phase.",
        "",
        "## Authorized Scope",
        "",
        "L1 policy validation and Yocto metadata-input missing-status reporting are authorized by default. L2 non-artifact scaffolding is authorized only with `WUCIOS_PHASE3CD_ALLOW_L2_SCAFFOLD=1` and an explicit scaffold target or runner flag.",
        "",
        "## Not Authorized",
        "",
        "L3 substrate artifact attempts and L4 runtime inspection are not authorized. BitBake execution, build-environment initialization, Yocto builds, source clones, layer downloads, rootfs generation, image generation, container actions, VM actions, package installation, and scoring remain forbidden.",
        "",
        "## Candidate Summary",
        "",
        *render_candidate_table(report["candidates"]),
        "",
        "## Yocto Metadata Policy",
        "",
        "Phase 3C-D records metadata input, layer, recipe, machine, distro, image target, source mirror, cache, output, and evidence requirements. Direct-rootfs and store-root assumptions are not reused as Yocto execution policy.",
        "",
        "## Future Evidence Requirements",
        "",
        "Future L3 Yocto attempts require metadata input manifests, layer and recipe manifests, machine and distro configuration, image target policy, download and mirror policy, build command and environment records, BitBake logs, artifact manifests, artifact hashes, package manifests, static Noether checks, Godel boundary notes, substrate reports, failure reports, and missing-measurement records. Runtime-only measurements remain `NOT_MEASURED_RUNTIME_REQUIRED` until runtime inspection is explicitly authorized.",
        "",
        "## Static Tool Findings",
        "",
        f"BitBake: `{detection_summary.get('bitbake', 'NOT_MEASURED')}`; devtool: `{detection_summary.get('devtool', 'NOT_MEASURED')}`; kas: `{detection_summary.get('kas', 'NOT_MEASURED')}`; repo: `{detection_summary.get('repo', 'NOT_MEASURED')}`; Docker: `{detection_summary.get('docker', 'NOT_MEASURED')}`; Podman: `{detection_summary.get('podman', 'NOT_MEASURED')}`; Buildah: `{detection_summary.get('buildah', 'NOT_MEASURED')}`; QEMU system: `{detection_summary.get('qemu-system-x86_64', 'NOT_MEASURED')}`.",
        "",
        "## Guardrail Results",
        "",
    ]
    ...
    lines.extend([
        ...
        "",
        "No substrate is selected in Phase 3C-D. Yocto preparation readiness is not substrate ranking.",
        "",
        "## Score Statement",
        "",
        "No numeric WuciOS score is generated in Phase 3C-D because no current WuciOS artifact and complete artifact-bound evidence exist.",
        "",
        "## Boundary Statement",
        "",
        "No substrate artifact attempt was made. No rootfs was generated. No image was generated. No Yocto build was attempted. No BitBake execution occurred. No build environment was initialized. No runtime inspection was attempted. No container was built or run. No image was pulled. No networked build was performed. No source tree was cloned. No layer was downloaded. No OS image was downloaded. No sudo was used. No package installation was attempted.",
    ])
    return "\n".join(lines)
...
def main() -> int:
    ...
    if args.l2_scaffold and os.environ.get(AUTH_ENV) != "1":
        print("L2_YOCTO_SCAFFOLD_NOT_AUTHORIZED", file=sys.stderr)
        return 1
    if report.get("global_status") == "PHASE_3C_D_BLOCKED":
        return 1
    print(f"Phase 3C-D: {report['global_status']}")
